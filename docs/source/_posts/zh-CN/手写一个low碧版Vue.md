...
comments: true
lang: zh-CN
summary: 手写一个low碧版Vue
tags: [Vue]
categories: 中级教程
abbrlink: 247cae9a
date: 2020-06-28 10:50:21
updated:
password:
---
# 手写一个low碧版Vue
...
```JS
1.new Vue()之后 Vue会调用初始化也就是_init()函数 初始化了生命周期、时间、props、data、computed、与watch等
...
class Wvue {
    constructor(options){
        ...
        this.$data = options.data
        this.observe(this.$data)
        //手动测试 当然 这里的watcher创建 是在我们的编译器里执行的 扫描视图上的语法双花括号等时候创建watcher
...
2.判断如果是文本节点 解析双花括号 如果有w-开头 或者 @的 就解析 指令或者事件
```
...
```JS
1.解析双花括号文本
2.把双花括号里面的值 替换成我们this.$data里赋值
3.这里我们用了正则匹配 并且 使用 RegExp.$1来取出双花括号里面的值
```

```JS
//解析文本节点 看看有每有{{xxx}}的然后 把值替换成我们Wvue里的变量 创建watcher
isInterpolation(node){
    //如果你的文本类型 是3 就说明你是文本节点 并且要满足正则匹配 你里面有双花括号的语法 
...
compileText(node){
        ...
        this.updated(node,this.$vm,exp,'text') //这里我么已经知道了 你是解析双花括号的方法 所以我传进去text
    }
    //这个通用函数 通过动态匹配 就知道调取我们的textUpdator方法 去改变双花括号里面的值
    updated (node,vm,exp,dir) {
        //这里我们可能是解析双花括号 也可能是解析 w-text 或者 w-html 这里我们写一个通用的函数 通过传参去匹配调用
    ...
    textUpdator(node,value){
        //替换界面中的双花括号里面的内容 为我们Vue里面data里面的键值
...
```js
1.我们创建watcher的时机是在 解析双花括号括号的时候创建的 也就是我们的
2.创建的watcher 我们需要手动访问一下Vue里data里的键 手动触发一下 get函数
3.当我们在给我们的Vue中data里的键改变键值的时候 触发set函数 然后dep通知更新 调用了watcher实例里的update函数
4.updata函数里面 是我们在解析双花括号解析双花括号的时候 传进去一个回调函数 这个函数 是我们刚刚上面解析双花括号括号的时候把里面的值替换为我们vue里data键值的textUpdator()函数
```
